  ...
  export default {
    ...
    methods: {
      ...
      getData() {
        ...
        request.post('/api/student/searchLeaveForm', form).then(res => {
      ...
      agree(row) {
        ...
        this.$confirm('确定同意退寝？', '提示', {type: "warning"}).then(() => {
          this.request.post('/api/student/agreeLeave', data).then(res => {
            if (!res.data.errcode) {
              this.$alert('同意退寝成功！', '提示', {type: "success"});
              this.getData();
            } else {
      ...
      reject(row) {
        const data = {id: row._id};
        this.$confirm('确定拒绝退寝？', '提示', {type: "warning"}).then(() => {
          this.request.post('/api/student/rejectLeave', data).then(res => {
            if (!res.data.errcode) {
              this.$alert('拒绝退寝成功！', '提示', {type: "success"});
              this.getData();
            } else {
              this.$alert(res.data.msg, '错误', {type: "error"});
            }
      ...
      remove(row) {
        const data = {id: row._id};
        this.$confirm('确定要删除该退寝单？', '提示', {type: "warning"}).then(() => {
          this.request.post('/api/student/removeLeave', data).then(res => {
            if (!res.data.errcode) {
