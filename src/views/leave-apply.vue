...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getSubmittedData() {
        this.request.post('/api/student/studentSubmittedLeaveForm').then(res => {
          if (!res.data.errcode) {
      ...
      async submit() {
        try {
          if (this.userInfo.reside_status !== 1) {
            this.$alert("您无需申请退寝", "提示", {type: "warning"});
            return false;
          }
          await this.$refs.form.validate();
          const data = {...this.form};
          this.request.post('/api/student/postLeaveForm', data).then(res => {
            if (!res.data.errcode) {
